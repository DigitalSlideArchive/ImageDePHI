...
const page = ref(1);
const rows = ref(10);
const totalPages = computed(() => Math.ceil(imageRedactionPlan.value.total / rows.value));

const updateImageList = async () => {
  const limit = rows.value;
  const offset = (page.value - 1) * limit;
  imageRedactionPlan.value = await getRedactionPlan(selectedDirectories.value.inputDirectory,rows.value, offset);
};
